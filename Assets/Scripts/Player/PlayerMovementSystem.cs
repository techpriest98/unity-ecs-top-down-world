...
namespace Game.Player
{
    ...
    [UpdateAfter(
        typeof(ChunkGenerationSystem))]
    public partial struct PlayerMovementSystem :
        ISystem
    {
        private const int InitialChunkCapacity =
            64;

        private const float Gravity =
            -30f;

        private const float MaximumFallSpeed =
            -50f;

        private const float MaximumVerticalStep =
            0.25f;

        private const int CollisionSearchIterations =
            8;

        private const float GroundProbeDistance =
            0.02f;

        private NativeParallelHashMap<
            int2,
        ...
        {
            float3 groundProbePosition =
                position;

            groundProbePosition.y -=
                GroundProbeDistance;

            return PlayerCollisionUtility
                .IsPositionBlocked(
                    groundProbePosition,
                    shape,
                    blockAccessor);
        ...
        private void UpdateChunkLookup(
            ref SystemState state)
        {
